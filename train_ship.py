...
from data_loader import get_cifar,get_ship
...
def parse_arguments():
    ...
    parser.add_argument('--dataset', default='ship', type=str, help='dataset. can be either cifar10 or cifar100')
...
if __name__ == "__main__":
    ...
    args.teacher=config['teacher']
    args.student=config['student']
    #print(args.teacher,args.student)
    print(args)
    trial_id = os.environ.get('NNI_TRIAL_JOB_ID')
    dataset = args.dataset
    num_classes = 14#100 if dataset == 'cifar100' else 'cifar10'
    teacher_model = None
    student_model = create_cnn_model(args.student, dataset, use_cuda=args.cuda,num_classes=num_classes)
    train_config = {
        'epochs': args.epochs,
    ...
    if args.teacher:
        teacher_model = create_cnn_model(args.teacher, dataset, use_cuda=args.cuda,num_classes=num_classes)
        if args.teacher_checkpoint:
            print("---------- Loading Teacher -------")
            teacher_model = load_checkpoint(teacher_model, args.teacher_checkpoint)
        else:
            print("---------- Training Teacher -------")
            train_loader, test_loader = get_ship(num_classes=num_classes,dataset_dir=args.dataset_dir)
